...
function trace(mask: Uint8Array, width: number, height: number, options: TraceOptions): { d: string; contours: number } { const stride = width + 1; const starts: number[] = [], ends: number[] = []; const map = new Map<number, number[]>(); const key = (x: number, y: number): number => y * stride + x; const pt = (k: number): Point => ({ x: k % stride, y: Math.floor(k / stride) }); const on = (x: number, y: number): boolean => x >= 0 && y >= 0 && x < width && y < height && mask[y * width + x] === 1; const add = (x0: number, y0: number, x1: number, y1: number): void => { const s = key(x0, y0), e = key(x1, y1), i = ends.length; starts.push(s); ends.push(e); const b = map.get(s); if (b) b.push(i); else map.set(s, [i]); };
